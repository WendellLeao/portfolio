...
This is Wendell Leão's professional portfolio. He's a Senior Unity Developer (5 years experience). The résumé PDF at `public/resume/resume.pdf` is the primary source of truth for anything requiring career facts, project descriptions, or copy accuracy (about-me text, synopses/descriptions in the locale JSONs, resume-adjacent claims) — read it directly for the fullest, most current detail. The summary below is a quick-reference distillation of it; ask before inventing/guessing details not covered in either.
...
- **BGNS Studios** (Serbia), Senior Unity Developer, May 2024–present — *Draft Fever Bowl* (`DraftGameItem`): frontend technical rep for a four-developer team, led onboarding of 4 programmers, architected scouting/drafting systems (2,000+ synced entities, 40→80-100 FPS), led HDRP→URP migration (menus to 80 FPS, gameplay 20-30→60 FPS), rebuilt character-generation pipeline (4min→~50s loads), fixed async texture-gen frame drops via UniTask/semaphore synchronization. *The Platform* (`ThePlatform`): joined as senior reinforcement on a Roblox/Rec Room-style UGC platform, fixed a critical FishNet late-joiner crash (duplicate object IDs from server-replicated object caching), deployed/managed dedicated Linux servers via PlayFlow Cloud for cross-platform (PC/Mobile/VR) multiplayer testing. *PIX Football Manager* (`PixGameItem`): Swagger-mapped REST integration, UI Toolkit diegetic interfaces.
- **Triplano Games** (Brazil), Mid-Level, Jan–May 2024 — *Amazon Warriors* (`AmazonGameItem`): sole programmer with full technical autonomy, expanded terrain generation, UI, character abilities, enemy AI, backend integration, monetization, and analytics, maintaining stable 30 FPS mobile.
